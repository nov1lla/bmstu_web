...
namespace GUI.MVP.Main
{
    public class ClientPresenter
    {
        ...
        public ClientPresenter(IMainFormView view,
                               int userId,
        ...
        {
            _view.DisplayChildForm(new FormViewProductsByClient(_userId, _productService, _cartService, _itemCartService));
        }

        private void OnOrdersViewRequested(object? sender, EventArgs e)
        {
            _view.DisplayChildForm(new FormViewOrdersByClient(_userId, _orderService, _userService, _promoService, _productService, _itemOrderService));
        }

        private void OnPromosViewRequested(object? sender, EventArgs e)
        {
            _view.DisplayChildForm(new FormViewPromosByClient(_userId, _promoService));
        }
