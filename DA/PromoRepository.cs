...
namespace DA
{
    public class PromoRepository : IPromoRepository
    {
        ...
        {
            return $"select exists (select * from PromoDB where id = {promo.Id} and code = '{promo.Code}' and discount = {promo.Discount} and data_start = '{promo.Data_start}' and data_end = '{promo.Data_end}')";
        }
